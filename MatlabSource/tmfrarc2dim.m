...
%               회전관성과 전단변형을 고려 가능함
%              ( 매트랩 파일 : tmfrarc2dim.m )    2016. 03. 19
%              
%====================================================================
function tmfrarc2dim
%
clear all
%fid1=fopen('C:\tmfrarc2dim.out','w'); % 파일 출력
%
% <입력> 계산 모델
%it=1; mt=0; % 일정단면 아크 (회전관성과 전단변형을 무시, 고정-고정) Ingenieur-Archiv (1979) 337-346 Fig.2
%it=1; mt=1; % 일정단면 티모센코 아크 (회전관성과 전단변형을 고려, 좌단-고정) Ingenieur-Archiv (1979) 337-346 Fig.2
...
for alpha_degree=ALPHA(1):ALPHA(2):ALPHA(3)
   ...
   [Hz]=bsc(FRQ,fno,modeYN,itn,nn,ndof,Theta,b0,h0,R,sy0,emod,roh,bc_no,KK,mt,kappa,nu)
   Hzs(ii,1:fno)=Hz;
end
%timeend=clock;
...
%
%---------------------------------------------------------------------------------------------------------------------------
function [Hz]=bsc(FRQ,fno,modeYN,itn,nn,ndof,Theta,b0,h0,R,sy0,emod,roh,bc_no,KK,mt,kappa,nu)
%---------------------------------------------------------------------------------------------------------------------------
%
% 이분법
%
...
bmin=fmin;  w=fmin;
[isp]=tmmfree(w,itn,nn,ndof,Theta,b0,h0,R,sy0,emod,roh,bc_no,KK,mt,kappa,nu);
%
fno_cal=0;
while ((w <= fmax) & (fno_cal < fno))
   w=w+fdel;
   [is]=tmmfree(w,itn,nn,ndof,Theta,b0,h0,R,sy0,emod,roh,bc_no,KK,mt,kappa,nu);
   if is ~= isp
      bmax=w;
...
%
%-----------------------------------------------------------------------
function [is]=tmmfree(w,itn,nn,ndof,Theta,b0,h0,R,sy0,emod,roh,bc_no,KK,mt,kappa,nu)
%-----------------------------------------------------------------------
%
% 전달행렬법을 이용한 자유진동해석
%
...
%
for ii=2:nn
   [FM]=make_fm(ii,ndof,itn,Theta,w,b0,h0,R,sy0,emod,roh,mt,kappa,nu);
   TM=FM*TM;
...
%
%-----------------------------------------------------------------------
function [FM]=make_fm(ii,ndof,itn,Theta,w,b0,h0,R,sy0,emod,roh,mt,kappa,nu)
%-----------------------------------------------------------------------
%
% 필드행렬 만들기 (일정단면 아크)
%
...
dh=dTheta/itn;
[FM]=multi_runge_kutta_gill(ndof,Y0,dh,itn,w,b0,h0,R,sy0,emod,roh,mt,kappa,nu);
%
%-----------------------------------------------------------------------
function [Y0]=multi_runge_kutta_gill(ndof,Y0,dh,itn,w,b0,h0,R,sy0,emod,roh,mt,kappa,nu)
%-----------------------------------------------------------------------
%
...
%
for ii=1:itn
   [UM]=make_um(w,b0,h0,R,sy0,emod,roh,mt,kappa,nu);
...
%
%-----------------------------------------------------------------------
function [U]=make_um(w,b0,h0,R,sy0,emod,roh,mt,kappa,nu)
%-----------------------------------------------------------------------
%
